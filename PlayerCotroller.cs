    ...
    private bool IsOnSlope()
    {
        ...
        if (Physics.Raycast(ray, out slopeHit, slopeRayDistance, groundLayer))
        {
            if (slopeHit.point.y >= transform.position.y) // repair
            {
                ...
                Debug.Log("slopeHit" + frontSlopeCheck.position);
            } else if (slopeHit.point.y <= transform.position.y)
            {
                frontSlopeCheck.position = transform.TransformDirection(transform.position.x, transform.position.y + 0.5f, transform.position.z);
                Debug.Log("transform" + frontSlopeCheck.position);
            } else
            {
                frontSlopeCheck.position = transform.TransformDirection(transform.position.x, transform.position.y + 0.5f, transform.position.z + 0.1f);
                Debug.Log("empty" + frontSlopeCheck.position);
            }
